...
with tab_who:
    ...
    keyword = st.text_input(
        "Search ANY health topic (e.g., 'deadly viruses', 'viral outbreaks', 'diabetes', 'heart disease'):",
        key="who_kw"
    ).lower()
    ...
    if keyword:
        ...
        # SECOND: fuzzy expand search
        fuzzy_matches = fuzzy_match(keyword, indicators)

        # Combine, keep unique results
        ...
        if results:
            ...
            st.warning("No exact matches. Try simplifying your search term.")
            st.info("TIP: Examples: 'virus', 'viral disease', 'mortality', 'cancer', 'diabetes'")

    else:
        st.info("Enter any natural-language term to search WHO indicators.")
