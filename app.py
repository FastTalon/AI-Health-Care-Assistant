...
def main():
    ...
    with tab_who:
        ...
        keyword = st.text_input("Keyword (e.g., 'diabetes','mortality','life')", key="who_kw").lower()
        indicators = who_list_indicators()

        if keyword:
            filtered = [i for i in indicators if keyword in i.get("IndicatorName","").lower()]
            if filtered:
                label_map={f"{i['IndicatorCode']} - {i['IndicatorName']}":i["IndicatorCode"] for i in filtered}
                sel = st.selectbox("Select Indicator", list(label_map.keys()))
                code = label_map[sel]
                if st.button("Fetch WHO Data", key="who_btn"):
                    st.json(who_search(code))
            else:
                st.warning("No indicators found for that keyword.")
        else:
            st.info("Enter a keyword to search WHO indicators.")
