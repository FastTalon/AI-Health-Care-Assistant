...
# =====================
# PubMed + WHO SEARCH UTILITIES (Option C)
# =====================
...
def pubmed_search(query, max_results=5):
    try:
        ...
        r = requests.get(url, params={"db":"pubmed","term":query,"retmax":max_results,"retmode":"json"}, timeout=15)
...
def who_search(indicator):
    try:
        ...
        if r.status_code!=200:
            return {"error":f"WHO API returned status {r.status_code}. Check indicator."}
...
def who_list_indicators():
    try:
        r = requests.get(WHO_BASE + "Indicator", timeout=20)
        if r.status_code!=200:
            return []
        return r.json().get("value",[])
    except:
...
# ========================================================================
# >>> RESTORED ORIGINAL TAB RENDERING FUNCTIONS <<<
# 
# These functions MUST be defined BEFORE def main(): is called.
# ========================================================================

def render_chat_tab():
    """Renders the main Chat Assistant UI."""
    pass

def render_appointments_tab():
    """Renders the Appointments UI."""
    pass

def render_files_tab():
    """Renders the Patient Files & Search UI."""
    pass

def render_metrics_tab():
    """Renders the Metrics & Logs UI."""
    pass

def render_debug_tab():
    """Renders the Debug Trace UI."""
    pass

# ========================================================================
# >>> END RESTORED ORIGINAL TAB RENDERING FUNCTIONS <<<
# ========================================================================

def main():
    st.title("🏥 Agentic Healthcare Assistant")
    st.caption(
        "Multi-agent AI workflow: onboarding, appointments, patient docs, disease info, "
        "summaries, PubMed/WHO retrieval, plus LLMOps logs and trace."
    )
    ...
    st.sidebar.text_input("Eval Model", value=EVAL_MODEL_NAME, disabled=True)
    st.sidebar.text_input("Embed Model", value=EMBED_MODEL_NAME, disabled=True)
    ...
    st.session_state.rag_mode = st.sidebar.radio("RAG Mode", rag_options)

    st.sidebar.markdown("---")
    st.sidebar.subheader("Appointment Settings")
    ...
    with tab_pubmed:
        st.subheader("🔎 PubMed Search (Formatted Results)")

        query = st.text_input("Enter topic:", key="pubmed_query")
        max_results = st.slider("Max results", 1, 20, 5, key="pubmed_max")

        if st.button("Search PubMed", key="pubmed_btn"):
            if not query.strip():
                st.warning("Please enter a search term.")
            else:
                results = pubmed_search(query.strip(), max_results=max_results)
                ...
                if not results:
                    ...
                    for item in results:
                        ...
                        st.markdown("---")
                        st.markdown(f"### 📝 {item.get('title', 'No title available')}")

                        st.write(f"**Journal:** {item.get('journal', 'N/A')}")
                        st.write(f"**Date:** {item.get('pubdate', 'N/A')}")
...
def fuzzy_match(keyword, indicator_list, threshold=0.35):
    """Return fuzzy-matched WHO indicators based on similarity scoring."""
    keyword = keyword.lower()
    scored = []
    for item in indicator_list:
        name = item.get("IndicatorName", "").lower()
        score = difflib.SequenceMatcher(None, keyword, name).ratio()
        if score >= threshold:
            scored.append((score, item))
    scored.sort(key=lambda x: x[0], reverse=True)
    return [item for score, item in scored]

    # NOTE: nothing should be indented here
    # Your WHO UI must NOT be placed inside this function!


    with tab_who:
    st.subheader("🌍 WHO Global Health Indicator Search")
    st.caption("Search using plain English (e.g., 'diabetes', 'life expectancy').")

    keyword = st.text_input("Enter a health topic:", key="who_keyword").lower().strip()

    indicator_list_url = "https://ghoapi.azureedge.net/api/Indicator?$top=5000"
    raw = requests.get(indicator_list_url).json()
    indicators = raw.get("value", [])
    ...
    if keyword and matched:
        st.success(f"Found {len(matched)} WHO indicators related to '{keyword}'")

        choice = st.selectbox(
            "Select an indicator",
            [f"{name} ({code})" for name, code in matched],
            key="who_choice"
        )
        selected_code = choice.split("(")[-1].replace(")", "").strip()

        country_filter = st.text_input("Filter by country code (optional, e.g., USA, CAN, GBR):", "").upper().strip()

        if st.button("Get WHO Data", key="who_fetch"):
            data = who_search(selected_code)

            if isinstance(data, dict) and "error" in data:
                st.error("The WHO API did not return data for this indicator.")
            else:
                filtered = data
                if country_filter:
                    filtered = [d for d in data if d.get("SpatialDim","") == country_filter]
                    if not filtered:
                        st.warning(f"No data found for country code: {country_filter}")
                        filtered = data

                if isinstance(filtered, list) and len(filtered) > 0:
                    st.subheader("📘 Summary (Readable English)")
                    latest = filtered[0]

                    value = latest.get("NumericValue", "N/A")
                    country = latest.get("SpatialDim", "N/A")
                    year = latest.get("TimeDim", "N/A")

                    readable = (
                        f"**Indicator:** {choice}

"
                        f"In **{country}**, during **{year}**, "
                        f"the reported value was **{value}**.

"
                        "This information comes directly from the WHO Global Health Observatory."
                    )
                    st.markdown(readable)

                    st.subheader("📈 Trend Over Time")
                    import matplotlib.pyplot as plt
                    years = [d.get("TimeDim") for d in filtered if d.get("NumericValue") is not None]
                    vals = [d.get("NumericValue") for d in filtered if d.get("NumericValue") is not None]
                    ...
                    st.markdown("---")
                    st.caption("Full WHO raw data (for debugging):")
                    st.json(filtered)
                else:
                    st.warning("WHO returned no usable data for this indicator.")

    elif keyword:
        st.warning("No WHO indicators found for that keyword.")

    else:
        st.info("Enter a keyword above to search WHO indicators.")
