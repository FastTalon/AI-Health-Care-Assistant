...
# =====================
# PubMed + WHO LIVE SEARCH INTEGRATIONS
# =====================
...
# Reset form fields on next run
    ...
    with st.form(key="appointment_form"):
        patient_name = st.text_input("Patient name", value=st.session_state.get("form_patient_name", ""))
...
def main():
    # Handle one-shot reset of appointment form fields
    if st.session_state.get("reset_appointment_form") is True:
        st.session_state.reset_appointment_form = False
    ...
    st.sidebar.text_input(
        "Model Name",
        value=MODEL_NAME,
        key="model_name_display",
        disabled=True,
    )
    st.sidebar.text_input(
        "Eval Model",
        value=EVAL_MODEL_NAME,
        key="eval_model_display",
        disabled=True,
    )
    st.sidebar.text_input(
        "Embed Model",
        value=EMBED_MODEL_NAME,
        key="embed_model_display",
        disabled=True,
    )
    ...
    # Main tabs – now includes PubMed & WHO search for Option C
    ...
    # PubMed live search UI
    with tab_pubmed:
        st.subheader("🔎 Live PubMed / MEDLINE Search")
        query = st.text_input("Enter a medical keyword or topic:", key="pubmed_query")
        max_results = st.slider("Max results", 1, 20, 5, key="pubmed_max_results")
