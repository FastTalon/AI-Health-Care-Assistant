...
def fuzzy_match(keyword, indicator_list, threshold=0.35):
    ...
    st.subheader("🌍 WHO Indicator Search Engine (Fuzzy Matching Enabled)")

    keyword = st.text_input(
        "Search ANY health topic (e.g., 'deadly viruses', 'viral outbreaks', 'diabetes', 'mortality'):",
        key="who_kw"
    ).lower()
    ...
    if keyword:
        substring_matches = [
            ...
            if keyword in i.get("IndicatorName", "").lower()
        ]

        fuzzy_matches = fuzzy_match(keyword, indicators)

        combined = {i["IndicatorCode"]: i for i in substring_matches + fuzzy_matches}
        results = list(combined.values())

        if results:
            st.success(f"Found {len(results)} relevant WHO indicators.")

            label_map = {
                f"{i['IndicatorCode']} – {i['IndicatorName']}": i["IndicatorCode"]
                for i in results
            }

            selection = st.selectbox("Select an Indicator", list(label_map.keys()))
            code = label_map[selection]

            if st.button("Fetch WHO Data", key="who_btn"):
                st.json(who_search(code))

        else:
            st.warning("No indicators matched that keyword.")
            st.info("Try words like: virus, viral, outbreak, mortality, cancer, diabetes")

    else:
        st.info("Enter a natural-language keyword to search WHO indicators.")
