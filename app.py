...
def main():
    ...
    with tab_who:
        ...
        st.caption(
            "Enter a WHO GHO indicator code, e.g., `WHOSIS_000015` (Life expectancy at birth, total)."
        )
        indicator = st.text_input("WHO Indicator Code:", key="who_indicator")
        if st.button("Search WHO", key="who_search_btn"):
            if not indicator.strip():
                st.warning("Please enter an indicator code.")
            else:
                data = who_search(indicator.strip())
                st.json(data)
